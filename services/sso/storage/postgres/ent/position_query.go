...
func (_q *PositionQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Position, init func(*Position), assign func(*Position, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(position.UsersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_position
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_position" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_position" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
